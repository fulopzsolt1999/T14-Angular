...
@Component({
  selector: 'app-tablazat-generalas',
  templateUrl: './tablazat-generalas.component.html',
  styleUrl: './tablazat-generalas.component.css'
})
export class TablazatGeneralasComponent {
  eredmenyek: string[] = [
  ...
  ObjektumFeltolto(feltoltendoElem: string[]): versenyEredmeny[] {
    ...
    for (let i: number = 0; i < feltoltendoElem.length; i++) {
      ...
      let ujObjektum: versenyEredmeny = {
  ...
  megjelenitendoAdatok: versenyEredmeny[] = this.ObjektumFeltolto(this.eredmenyek);


  //Adat feltöltése a táblázatba
  helyezesErtek!: number;
  meretErtek!: number;
  sportagErtek!: string;
  versenyszamErtek!: string;

  ujEredmenyHozzaadas(): void {
    this.megjelenitendoAdatok.push(
      {
        helyezes: this.helyezesErtek,
        meret: this.meretErtek,
        sportag: this.sportagErtek,
        versenyszam: this.versenyszamErtek
      }
    )
  }

  //Szűrő mező használata

  keresoErtek: string = "";

  get szurtElemek() {
    if (!this.keresoErtek) {
      return this.megjelenitendoAdatok;
    }
    return this.megjelenitendoAdatok.filter(megjelenitendoAdatok => megjelenitendoAdatok.sportag.toLowerCase().includes(this.keresoErtek.toLowerCase()) ||
      megjelenitendoAdatok.versenyszam.toLowerCase().includes(this.keresoErtek.toLowerCase())
    )
  }
